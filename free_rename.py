    ...
    def export_list(self) -> None:
        ...
        try:
            ...
            if path.suffix.lower() == ".csv":
                lines = ["原文件名,预览新文件名,状态,扩展名,所在目录"]
                for item, new_name, state, ext, folder in self.preview_rows:
                    row = [item.name, new_name or "", state, ext, folder]
