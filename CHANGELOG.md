# Changelog

## 1.0.14

- 所有 Python 源码统一移入 `src/` 目录，根目录保留文档、配置与构建脚本
- `run_free_rename.bat` 改为从 `src/free_rename.py` 启动
- `free_rename.spec`、`build_resources.py`、`sync_version.py`、构建脚本全部适配新的 `src/` 结构
- Qt 资源编译输出调整为 `src/resources_rc.py`，根目录不再混放 Python 文件

## 1.0.13

- PyInstaller 打包改为 onedir，降低启动时的自解压开销
- 新增 sync_version.py，以 version_info.txt 为单一版本源自动同步版本号
- 新增 resources.qrc / build_resources.py，支持将图标与 QSS 编译进 Qt 资源
- 构建脚本增强：自动同步版本、编译资源，并支持可选 UPX_DIR 压缩

## 1.0.12

- `ScanWorker` 目录扫描改为基于 `os.scandir`，递归导入大目录更快
- `PreviewRow` 调整为 `dataclass(slots=True)`，大批量预览时降低对象内存占用
- 排序阶段改为预构建排序键后再排序，减少重复字符串转换与属性读取

## 1.0.11

- 新增“排序后再编号”，支持当前顺序、文件名、修改时间、创建时间、扩展名，以及倒序
- 执行阶段继续复用当前预览顺序，排序后的预览与实际处理顺序一致
- 修正排序场景下的移除逻辑，改为按文件路径匹配删除
- 启用排序后会限制上移 / 下移，避免顺序逻辑冲突

## 1.0.8

- 预览后台线程新增进度回传，状态栏和进度条会显示当前预览推进情况
- 历史页新增搜索、导出、打开日志位置与清空功能
- 记住当前页面、规则页签、主分栏状态与预览表头宽度
- 大列表时减少列宽自动测量带来的刷新开销
- 启动时按尾部读取最近历史，日志变大后启动更稳

## 1.0.7

- 新增统一的“取消任务”按钮，支持取消预览、扫描与执行任务
- 执行取消时增强文件状态回退，严格模式整体回滚，容错模式回滚未写入项
- 历史记录自动落盘，软件重启后仍保留最近操作
- 关闭窗口时提示先取消后台任务，避免误关
- 程序与文档版本统一更新到 1.0.7

## 1.0.6

- 预览表格切换为 `QTableView + QAbstractTableModel`
- 文件夹扫描改为后台线程执行
- 执行阶段直接复用当前预览结果，不再同步重算规则
- 新增 `workers.py`，进一步分离 Worker 与文件管理逻辑
- 打包脚本与 spec 增加 `styles/` 资源

## 1.0.5

- 正式拆分为 `rule_engine.py`、`file_manager.py`、`ui_main.py`
- `遇错继续` 进入单文件容错模式
- 文件操作进一步下沉到 `FileManager`

## 1.0

- 整理为可直接上传 GitHub 的源码结构
- 统一版本号为 1.0
- 清理本机绝对路径与构建日志残留
- 重写 `free_rename.spec` 为相对路径
- 新增 `.gitignore`、`README.md`、`version_info.txt`
- 保留图标资源与 Windows 运行 / 打包脚本
